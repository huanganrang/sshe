...
<script type="text/javascript">
	...
	$(function() {
		dataGrid = $('#dataGrid').datagrid({
			url : '${pageContext.request.contextPath}/fmPropertiesController/dataGrid',
			fit : true,
			fitColumns : true,
			border : false,
			pagination : true,
			...
			columns : [ [ {
				...
				field : 'isdeleted',
				title : '<%=TfmProperties.ALIAS_ISDELETED%>',
				width : 50		
				}, {
				field : 'name',
...
<body>
			...
			<form id="searchForm">
				<table class="table table-hover table-condensed" style="display: none;">
						<tr>	
							...
							<td>
								<input type="text" class="span2" onclick="WdatePicker({dateFmt:'<%=TfmProperties.FORMAT_ADDTIME%>'})" id="addtimeBegin" name="addtimeBegin"/>
								<input type="text" class="span2" onclick="WdatePicker({dateFmt:'<%=TfmProperties.FORMAT_ADDTIME%>'})" id="addtimeEnd" name="addtimeEnd"/>
							</td>
							<th><%=TfmProperties.ALIAS_UPDATETIME%></th>	
							<td>
								<input type="text" class="span2" onclick="WdatePicker({dateFmt:'<%=TfmProperties.FORMAT_UPDATETIME%>'})" id="updatetimeBegin" name="updatetimeBegin"/>
								<input type="text" class="span2" onclick="WdatePicker({dateFmt:'<%=TfmProperties.FORMAT_UPDATETIME%>'})" id="updatetimeEnd" name="updatetimeEnd"/>
							</td>
						...
						<tr>	
							...
							<td>
											<input type="text" name="require" maxlength="0" class="span2"/>
							</td>
						</tr>	
						<tr>	
							<th><%=TfmProperties.ALIAS_DEFAULT_VALUE%></th>	
							<td>
											<input type="text" name="defaultValue" maxlength="200" class="span2"/>
							</td>
						</tr>	
				</table>
			</form>
	...
	<div id="toolbar" style="display: none;">
		...
		<c:if test="${fn:contains(sessionInfo.resourceList, '/fmPropertiesController/download')}">
