...
<html>
<head>
<title>FmAuthApply管理</title>
<jsp:include page="../inc.jsp"></jsp:include>
<c:if test="${fn:contains(sessionInfo.resourceList, '/fmAuthApplyController/editPage')}">
	<script type="text/javascript">
		$.canEdit = true;
	</script>
</c:if>
<c:if test="${fn:contains(sessionInfo.resourceList, '/fmAuthApplyController/delete')}">
	<script type="text/javascript">
		$.canDelete = true;
	</script>
</c:if>
<c:if test="${fn:contains(sessionInfo.resourceList, '/fmAuthApplyController/view')}">
	<script type="text/javascript">
		$.canView = true;
	</script>
</c:if>
<script type="text/javascript">
	...
	$(function() {
		dataGrid = $('#dataGrid').datagrid({
			url : '${pageContext.request.contextPath}/fmAuthApplyController/dataGrid',
			fit : true,
			fitColumns : true,
			border : false,
			pagination : true,
			...
			columns : [ [ {
				...
				field : 'userCard',
				title : '<%=TfmAuthApply.ALIAS_USER_CARD%>',
				width : 50		
				}, {
				field : 'authRemark',
				...
				formatter : function(value, row, index) {
					...
					if ($.canEdit && row.status == 'AU01') {
						str += $.formatString('<input type="button" class="button gray" onclick="editFun(\'{0}\');" value="审核"/>', row.id);
					}
					str += '&nbsp;';
					if ($.canDelete) {
	...
	function deleteFun(id) {
		...
		parent.$.messager.confirm('询问', '您是否要删除当前数据？', function(b) {
			if (b) {
				...
				$.post('${pageContext.request.contextPath}/fmAuthApplyController/delete', {
					id : id
				}, function(result) {
					if (result.success) {
	...
	function viewFun(id) {
		...
		parent.$.modalDialog({
			...
			width : 780,
			height : 500,
			href : '${pageContext.request.contextPath}/fmAuthApplyController/view?id=' + id
		});
	}
	...
	function addFun() {
		parent.$.modalDialog({
			...
			width : 780,
			height : 500,
			href : '${pageContext.request.contextPath}/fmAuthApplyController/addPage',
			buttons : [ {
				text : '添加',
				handler : function() {
					parent.$.modalDialog.openner_dataGrid = dataGrid;//因为添加成功之后，需要刷新这个dataGrid，所以先预定义好
	...
	function downloadTable(){
	    ...
	    $('#downloadTable').form('submit', {
	        url:'${pageContext.request.contextPath}/fmAuthApplyController/download',
...
<body>
	...
	<div id="toolbar" style="display: none;">
		<c:if test="${fn:contains(sessionInfo.resourceList, '/fmAuthApplyController/addPage')}">
		...
		<c:if test="${fn:contains(sessionInfo.resourceList, '/fmAuthApplyController/download')}">
