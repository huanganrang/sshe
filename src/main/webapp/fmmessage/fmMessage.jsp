...
<html>
<head>
<title>FmMessage管理</title>
<jsp:include page="../inc.jsp"></jsp:include>
<c:if test="${fn:contains(sessionInfo.resourceList, '/fmMessageController/editPage')}">
	<script type="text/javascript">
		$.canEdit = true;
	</script>
</c:if>
<c:if test="${fn:contains(sessionInfo.resourceList, '/fmMessageController/delete')}">
	<script type="text/javascript">
		$.canDelete = true;
	</script>
</c:if>
<c:if test="${fn:contains(sessionInfo.resourceList, '/fmMessageController/view')}">
	<script type="text/javascript">
		$.canView = true;
	</script>
</c:if>
<script type="text/javascript">
	...
	$(function() {
		dataGrid = $('#dataGrid').datagrid({
			url : '${pageContext.request.contextPath}/fmMessageController/dataGrid',
			fit : true,
			fitColumns : true,
			border : false,
			pagination : true,
	...
	function deleteFun(id) {
		...
		parent.$.messager.confirm('询问', '您是否要删除当前数据？', function(b) {
			if (b) {
				...
				$.post('${pageContext.request.contextPath}/fmMessageController/delete', {
					id : id
				}, function(result) {
					if (result.success) {
	...
	function viewFun(id) {
		...
		parent.$.modalDialog({
			...
			width : 780,
			height : 500,
			href : '${pageContext.request.contextPath}/fmMessageController/view?id=' + id
		});
	}
	...
	function downloadTable(){
	    ...
	    $('#downloadTable').form('submit', {
	        url:'${pageContext.request.contextPath}/fmMessageController/download',
...
<body>
	...
	<div id="toolbar" style="display: none;">
		<c:if test="${fn:contains(sessionInfo.resourceList, '/fmMessageController/addPage')}">
			<a onclick="addFun();" href="javascript:void(0);" class="easyui-linkbutton" data-options="plain:true,iconCls:'bug_add'">添加</a>
		</c:if>
		<a href="javascript:void(0);" class="easyui-linkbutton" data-options="iconCls:'brick_add',plain:true" onclick="searchFun();">过滤条件</a><a href="javascript:void(0);" class="easyui-linkbutton" data-options="iconCls:'brick_delete',plain:true" onclick="cleanFun();">清空条件</a>
		<c:if test="${fn:contains(sessionInfo.resourceList, '/fmMessageController/download')}">
